...
void Motor_SetLeftSpeed(int8_t Speed)
{
	...
	if (Speed > 0)							//如果设置正转的速度值
	{
		...
		GPIO_SetBits(GPIOB, GPIO_Pin_0);	//PB1置高电平
		GPIO_ResetBits(GPIOB, GPIO_Pin_1);	//PB0置低电平，设置方向为正转
	...
	else									//否则，即设置反转的速度值
	{
		...
		GPIO_ResetBits(GPIOB, GPIO_Pin_0);	//PB1置低电平
		GPIO_SetBits(GPIOB ,GPIO_Pin_1);	//PB0置高电平，设置方向为反转
		PWM_SetCompare1(-Speed);
	}
}
