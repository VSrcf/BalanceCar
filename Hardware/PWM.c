...
void PWM_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_11;       // 选择PA8(TIM1_CH1)和PA11(TIM1_CH4)
	...
	TIM_TimeBaseInit(TIM1,&TIM_TimeBaseInitStructure);             // 将配置写入TIM1
	...
	TIM_OC4Init(TIM2,&TIM_OCInitStructure);                        // 将配置写入TIM2通道4（控制右电机）
...
{
	TIM_SetCompare4(TIM2,Compare);                                 // 将比较值写入TIM2的CCR4寄存器
}
